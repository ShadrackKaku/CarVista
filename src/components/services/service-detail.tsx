...
/** The service-provider profile, shared by the public and in-shell pages. */
export async function ServiceDetail({ slug, inShell = false }: ServiceDetailProps) {
  const service = await getServiceBySlug(slug);
  if (!service) notFound();

  return (
    <div className={inShell ? "" : "container-page py-10"}>
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <aside className="space-y-5">
          <div className="sticky top-6 rounded-2xl border bg-card p-6 shadow-soft">

    ...
    return (
      <div className="container-page flex flex-col items-center justify-center py-24 text-center">
        ...
        <Button asChild variant="gradient" className="mt-6">
          <Link href="/app/marketplace/parts">Shop car parts</Link>
        </Button>
      </div>
    );
  }
            ...
            <div key={item.partId} className="flex gap-4 rounded-xl border bg-card p-4">
              ...
              <div className="flex flex-1 flex-col">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <Link href={`/app/marketplace/parts/${item.slug}`} className="font-semibold hover:text-brand-600">
                      {item.name}
                    </Link>
        ...
        <aside>
          <div className="sticky top-24 rounded-xl border bg-card p-6 shadow-soft">
            ...
            <Button asChild variant="gradient" className="mt-5 w-full" size="lg">
              <Link href="/app/marketplace/checkout">Proceed to checkout</Link>
            </Button>
            <Button asChild variant="ghost" className="mt-2 w-full">
              <Link href="/app/marketplace/parts">Continue shopping</Link>
            </Button>
          </div>
        </aside>
      </div>
